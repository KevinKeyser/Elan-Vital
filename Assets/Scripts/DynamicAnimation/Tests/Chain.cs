...
public class Chain : Bone {
    ...
    public void ReverseKinematics()
    {
        ...
        for (int index = ChildBones.Count - 1; index >= 0; index--)
        {
            ...
            {
                var differenceBetweenTargetAndLastBone = (ChildBones[index].Target.position - ChildBones[index].transform.position);
                var proportion = ChildBones[index].Length / differenceBetweenTargetAndLastBone.magnitude;

                var position = (1 - proportion) * ChildBones[index].Target.position + (proportion * ChildBones[index].transform.position);
                ChildBones[index].transform.position = position;
            }
            else
            {
                var differenceBetweenBones = (ChildBones[index + 1].transform.position - ChildBones[index].transform.position);
                var proportion = ChildBones[index].Length / differenceBetweenBones.magnitude;

                var position = (1 - proportion) * ChildBones[index + 1].transform.position + (proportion * ChildBones[index].transform.position);
                ChildBones[index].transform.position = position;
            }
        }
    }
    ...
    public void Solve()
    {
        ...
        if (distanceBetweenStartAndTarget > MaxLength)
        {
            ...
            for (int index = 0; index < ChildBones.Count - 1; index++)
            {
                var differenceBetweenTargetAndBone = (Target.position - ChildBones[index].transform.position).magnitude;
                var proportion = ChildBones[index].Length / differenceBetweenTargetAndBone;

                ChildBones[index + 1].transform.position = (1 - proportion) * ChildBones[index].transform.position + proportion * Target.position;
            }
        }
        else
        {
            int approximationCount = 0;
            var differenceBetweenLastBoneAndTarget = (ChildBones[ChildBones.Count - 1].transform.position - Target.position).magnitude;
            while(differenceBetweenLastBoneAndTarget > Tolerance)
            {
                // start with the reverse, then do forward 
                ReverseKinematics();
                ForwardKinematics();
                differenceBetweenLastBoneAndTarget = (ChildBones[ChildBones.Count - 1].transform.position - Target.position).magnitude;
                approximationCount += 1;
                // 10 approximatinos ought to suffice
                if(approximationCount > 100)
                {
                    break;
                }
            }
        ...
        for (int index = 0; index < ChildBones.Count; index++)
        {
            ...
            {
                ChildBones[index].transform.rotation = Quaternion.LookRotation(Target.position, Target.up);
            }
            else
            {
                ChildBones[index].transform.rotation = Quaternion.LookRotation(ChildBones[index + 1].transform.position, ChildBones[index + 1].transform.up);
